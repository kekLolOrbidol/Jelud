...
import com.valpu.seve.tetris.models.Puzzle
...
class TetView : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (model != null) {
            for (i in 0 until FieldConsts.ROW_COUNT.value) {
                for (j in 0 until FieldConsts.COLUMN_COUNT.value) {
                    drawCell(canvas, i, j)
                }
            }
        }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val cellWidth = (w - 2 * FRAME_OFFSET_BASE) / FieldConsts.COLUMN_COUNT.value
        val cellHeight = (h - 2 * FRAME_OFFSET_BASE) / FieldConsts.ROW_COUNT.value

        val n = Math.min(cellWidth, cellHeight)
        this.cellSize = Dimension(n, n)

        val offSetX = (w - FieldConsts.COLUMN_COUNT.value * n) / 2
        val offSetY = (h - FieldConsts.ROW_COUNT.value * n) / 2
        this.frameOffSet = Dimension(offSetX, offSetY)
    }

    private class ViewHandler(private val owner: TetView) : Handler() {
...
//                        val mpDieSound = MediaPlayer.create(owner.context, R.raw.die)
    ...
    private fun drawCell(canvas: Canvas, row: Int, column: Int) {
        ...
        if (CellConsts.EMPTY.value != cellStatus) {
            val color = if (CellConsts.EPHEMERAL.value == cellStatus) {
                model?.currentPuzzle?.getColor()
            } else {
                Puzzle.getColor(cellStatus as Byte)
            }
